...
function LoginPage() {
  ...
  const handleAuthSuccess = async (user) => {
    ...
    try {
      ...
      console.warn("Could not sync profile details, but login succeeded:", err);
    }
  ...
  return (
    <div className="landing-page">
      ...
      <div className="player-card glass" style={{ maxWidth: "420px", width: "100%", zIndex: 2, padding: "2.5rem" }}>
        <h2 style={{ color: "#e0f2fe", marginBottom: "0.5rem", textAlign: "center", fontSize: "2rem" }}>Welcome Back</h2>
        <p style={{ color: "#a1d5e6", marginBottom: "2rem", textAlign: "center", fontSize: "0.9rem" }}>Log in to continue your session</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%" }}>
          ...
          <button type="submit" disabled={loading} className="blend-start-btn" style={{ width: "100%", padding: "0.85rem", marginTop: "0.5rem", fontSize: "1rem", fontWeight: "600", opacity: loading ? 0.7 : 1, cursor: loading ? "not-allowed" : "pointer" }}>
            {loading ? <span className="spin-loader">⏳</span> : "Login"}
          </button>
        </form>
        ...
        <p style={{ marginTop: "1.5rem", color: "#a1d5e6", fontSize: "0.95rem", textAlign: "center" }}>
          Don't have an account? <Link to="/signup" style={{ color: "#38bdf8", textDecoration: "none", fontWeight: "600", pointerEvents: loading ? "none" : "auto" }}>Sign up</Link>
        </p>
      </div>
    </div>
  );
